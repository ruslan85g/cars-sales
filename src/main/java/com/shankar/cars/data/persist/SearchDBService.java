...
public class SearchDBService extends DBService {
	...
	public List<Car> load(Class<Car> class1, SearchMeta searchMeta) {
		...
		LoadType<Car> cars = ofy().load().type(class1);
		if (searchMeta.getCar_type_id() != null) {
			cars.filter("car_type_id ==", searchMeta.getCar_type_id());
		}
		if (searchMeta.getCar_model_id() != null) {

			cars.filter("car_model_id ==", searchMeta.getCar_model_id());
		}
		if (searchMeta.getYearF() != null) {
			cars.filter("yearF", searchMeta.getYearF());
		}
		if (searchMeta.getType_geare() != null
				&& !searchMeta.getType_geare().isEmpty()) {
			cars.filter("type_geare", searchMeta.getType_geare());
		}
		if (searchMeta.getVolume() != null && !searchMeta.getVolume().isEmpty()) {
			cars.filter("volume", searchMeta.getVolume());
		}
		if (searchMeta.getKm() != null) {
			cars.filter("km", searchMeta.getKm());
		}
		if (searchMeta.getColor() != null && !searchMeta.getColor().isEmpty()) {
			cars.filter("color", searchMeta.getColor());
		}
		if (searchMeta.getPriceF() != null) {
			cars.filter("priceF", searchMeta.getPriceF());
		}
		if (searchMeta.getText() != null && !searchMeta.getText().isEmpty()) {
			cars.filter("text", searchMeta.getText());
		}
		cars.list();
